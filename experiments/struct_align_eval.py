"""
R-GCN + Drug Structural Alignment Loss
=======================================
Adds an auxiliary loss that forces learned antibiotic embeddings to
preserve Tanimoto (Jaccard) fingerprint similarity between Morgan fingerprints.

  L_total = L_link + lambda * MSE(cosine_sim(z_ab), Tanimoto(fp_ab))

Inspired by the drug structural alignment loss in CrossContrastAMR (concurrent
zero-shot AMR work). Zero leakage: Tanimoto is computed from Morgan fingerprints
(chemical structure, always available), NOT from resistance labels.

Why this helps zero-shot: test antibiotics with fingerprints similar to training
antibiotics will land near their structural neighbors in embedding space, enabling
the model to score them accurately despite seeing no resistance edges for them.

Tested lambda values: 0.0 (vanilla R-GCN, sanity check), 0.1, 0.5
Same 5 seeds and evaluation protocol as multiseed_eval.py.
Existing results in multiseed_results.json are NOT modified.

Results saved to: results/metrics/struct_align_results.json
"""
...
PATIENCE = 4
PROJ_DIM = 64
LAMBDAS  = [0.0, 0.1, 0.5]   # 0.0 = vanilla R-GCN (sanity check)
...
ab_rich        = graph_data['antibiotic_rich_features']   # [231, 1024] Morgan FP
...
# ── Pre-compute Tanimoto similarity matrix from Morgan fingerprints ────────────
# Tanimoto(A, B) = (A·B) / (|A|_1 + |B|_1 - A·B)
# Morgan fingerprints are binary bit vectors stored as float32.
# This is purely structural chemistry — no resistance labels involved.
print("Pre-computing 231×231 Tanimoto matrix from Morgan fingerprints...")
with torch.no_grad():
    fp = ab_rich.float()                           # [231, 1024]
    dot = fp @ fp.t()                              # [231, 231]  A·B
    norms = fp.sum(dim=1, keepdim=True)            # [231, 1]    |A|_1
    denom = norms + norms.t() - dot                # [231, 231]  |A|+|B|-A·B
    tanimoto_matrix = (dot / denom.clamp(min=1e-8)).clamp(0., 1.)  # [231, 231]

print(f"Tanimoto matrix: mean={tanimoto_matrix.mean():.3f}  "
      f"max_offdiag={tanimoto_matrix.fill_diagonal_(0).max():.3f}")
tanimoto_matrix.fill_diagonal_(1.0)   # restore diagonal
...
print(f"Gene nodes: {len(gene_idx_list)}  |  Train ABs: {len(train_ab_idx)}  |  "
      f"Test ABs: {len(test_ab_idx)}  |  Lambda values: {LAMBDAS}")


# ── Feature projector (same as multiseed_eval.py) ─────────────────────────────
...
def struct_align_loss(z, ab_indices, tanimoto_mat):
    """
    MSE between cosine similarity of learned antibiotic embeddings
    and pre-computed Tanimoto fingerprint similarity matrix.

    Args:
        z: all node embeddings [N, dim]
        ab_indices: indices of all antibiotic nodes [231]
        tanimoto_mat: pre-computed Tanimoto matrix [231, 231]
    Returns:
        scalar loss
    """
    ab_embs = z[ab_indices]                                  # [231, dim]
    ab_norm = F.normalize(ab_embs, p=2, dim=-1)              # L2-normalize
    cos_sim = ab_norm @ ab_norm.t()                          # [231, 231]
...
for lam in LAMBDAS:
    label = f"R-GCN+StructAlign(lam={lam})" if lam > 0 else "R-GCN(lam=0, sanity)"
    print(f"\n{'='*70}")
    ...
    all_results[label] = {'lambda': lam, 'per_seed': seed_results, 'aggregated': agg}
...
# ── Final table ───────────────────────────────────────────────────────────────
print(f"\n\n{'='*90}")
print("STRUCTURAL ALIGNMENT ABLATION  (R-GCN, 5 seeds)")
print(f"{'='*90}")
...
# Summary: best lambda
best_label = max(
    [(k, v) for k, v in all_results.items() if v['lambda'] > 0],
    key=lambda x: x[1]['aggregated']['mrr']['mean']
)[0]
best_mrr = all_results[best_label]['aggregated']['mrr']['mean']
base_mrr = all_results['R-GCN(lam=0, sanity)']['aggregated']['mrr']['mean']
print(f"\nBest StructAlign variant: {best_label}")
print(f"MRR improvement over vanilla R-GCN: {best_mrr:.4f} vs {base_mrr:.4f} "
      f"({'+' if best_mrr >= base_mrr else ''}{(best_mrr-base_mrr)/base_mrr*100:.1f}%)")

out_path = Path(__file__).parent.parent / "results" / "metrics" / "struct_align_results.json"
